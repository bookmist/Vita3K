...
EXPORT(int, __aeabi_idiv, int32_t numerator, int32_t denominator) {
...
EXPORT(std::div_t, __aeabi_uidivmod, int32_t numerator, int32_t denominator) {
...
EXPORT(std::lldiv_t, __aeabi_uldivmod, int64_t numerator, int64_t denominator) {
...
EXPORT(Ptr<void>, kmemset, Ptr<void> dst, int val, SceSize size) {
    auto res = memset(dst.get(emuenv.mem), val, size);
    return dst;
}
