...
#define RET_NET_ERRNO(ret)                              \
    do {                                                \
        int _r = (ret);                                 \
        return (_r < 0 ? RET_ERROR(-(_r & 0xff)) : _r); \
    } while (0)
...
EXPORT(int, sceNetSyscallClose, int sid) {
    ...
    LOG_CONSOLE(sceNetSyscallClose, sid)
    auto sock = lock_and_find(sid, emuenv.net.socks, emuenv.kernel.mutex);
    if (!sock) {
        return -RET_ERROR(SCE_NET_EBADF);
    }
...
EXPORT(int, sceNetSyscallDumpRead, int id, void *buf, int len, void *pflags) {
    ...
    LOG_CONSOLE(sceNetSyscallDumpRead, id, buf, len, pflags);
    return UNIMPLEMENTED();
}

EXPORT(int, sceNetSyscallEpollAbort, int eid, int flags) {
    TRACY_FUNC(sceNetSyscallEpollAbort, eid, flags);
    LOG_CONSOLE(sceNetSyscallEpollAbort, eid, flags);
    return UNIMPLEMENTED();
}
...
EXPORT(int, sceNetSyscallSendmsg, int s, const SceNetMsghdr *msg, int flags) {
    ...
    if (msg->msg_iovlen == 1) {
        RET_NET_ERRNO(sock->send_packet(msg->msg_iov.get(emuenv.mem)->iov_base.get(emuenv.mem), msg->msg_iov.get(emuenv.mem)->iov_len, flags, msg->msg_name.get(emuenv.mem), msg->msg_namelen));
    } else {
        ...
        RET_NET_ERRNO(sock->send_packet(data.data(), total_data_size, flags, msg->msg_name.get(emuenv.mem), msg->msg_namelen));
    }
}
...
EXPORT(int, sceNetSyscallSendto, SceNetSyscallSendtoParam *param) {
    ...
    RET_NET_ERRNO(sock->send_packet(param->msg.get(emuenv.mem), param->len, param->flags, param->to.get(emuenv.mem), param->tolen));
}
...
EXPORT(int, sceNetSyscallShutdown, int s, SceNetShutdownMethod how) {
    ...
    RET_NET_ERRNO(sock->shutdown(how));
}
...
EXPORT(int, sceNetSyscallSysctl, SceNetSyscallSysctlParam *param) {
    TRACY_FUNC(sceNetSyscallSysctl, param->socketId, param->level, param->optname, param->optval, param->optlen, param->param_6)
    LOG_CONSOLE(sceNetSyscallSysctl, param->socketId, param->level, param->optname, param->optval, param->optlen, param->param_6)
    return UNIMPLEMENTED();
}
