...
static bool is_addcont_exist(EmuEnvState &emuenv, const SceChar8 *path) {
    const auto drm_content_id_path{ emuenv.pref_path / boost::describe::enum_to_string(VitaIoDevice::ux0, "") / emuenv.io.device_paths.addcont0 / reinterpret_cast<const char *>(path) };
