...
static void adhoc_thread(EmuEnvState &emuenv, int thread_id) {
    ...
    while (emuenv.netctl.adhocThreadRun.load()) {
        ...
        int recv_id = CALL_EXPORT(sceNetSocket, "SceNetAdhocAuthRecv", SCE_NET_AF_INET, SCE_NET_SOCK_DGRAM_P2P, SCE_NET_IPPROTO_IP);
        if (recv_id < 0) {
        ...
        int send_id = CALL_EXPORT(sceNetSocket, "SceNetAdhocAuthSend", SCE_NET_AF_INET, SCE_NET_SOCK_DGRAM_P2P, SceNetProtocol(0));
        if (send_id < 0) {
        ...
        while (emuenv.netctl.adhocCondVarReady.load()) {
            ...
            auto res = CALL_EXPORT(sceNetRecvfrom, recv_id, &peerInfo, sizeof(SceNetCtlAdhocPeerInfo), 0, (SceNetSockaddr *)&fromAddr, &fromlen);
            ...
            // If no packet was received, proactively send our peer info to ensure visibility on the network.
            if (res < 0)
                CALL_EXPORT(sceNetSendto, send_id, &selfInfo, sizeof(selfInfo), 0, (SceNetSockaddr *)&to, sizeof(to));

            // Sleep 264ms between iterations to prevent busy-waiting and reduce CPU usage
            std::this_thread::sleep_for(std::chrono::milliseconds(264));
        }
