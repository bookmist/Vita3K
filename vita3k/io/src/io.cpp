...
bool init(IOState &io, const fs::path &cache_path, const fs::path &log_path, const fs::path &pref_path, bool redirect_stdio) {
    ...
    boost::mp11::mp_for_each<boost::describe::describe_enumerators<VitaIoDevice>>([pref_path](auto i) {
        if (device::is_valid_output_path(i.value))
            fs::create_directories(pref_path / i.name);
    });

    const fs::path ux0{ pref_path / boost::describe::enum_to_string(VitaIoDevice::ux0, "") };
    const fs::path uma0{ pref_path / boost::describe::enum_to_string(VitaIoDevice::uma0, "") };
    const fs::path vd0{ pref_path / boost::describe::enum_to_string(VitaIoDevice::vd0, "") };
...
bool init_savedata_app_path(IOState &io, const fs::path &pref_path) {
    const fs::path user_id_path{ pref_path / boost::describe::enum_to_string(VitaIoDevice::ux0, "") / "user" / io.user_id };
...
std::string translate_path(const char *path, VitaIoDevice &device, const IOState::DevicePaths &device_paths) {
    ...
    case VitaIoDevice::cache0: { // Redirect cache0: to ux0:cache/<title_id>
        relative_path = device::remove_device_from_path(relative_path, device, "cache/" + device_paths.app0);
        device = VitaIoDevice::ux0;
        break;
    }
...
SceUID open_file(IOState &io, const char *path, const int flags, const fs::path &pref_path, const char *export_name) {
    ...
    if ((device == VitaIoDevice::tty0) || (device == VitaIoDevice::tty1) || (device == VitaIoDevice::tty2) || (device == VitaIoDevice::tty3)) {
        ...
        LOG_TRACE_IF(log_file_op, "{}: Opening terminal {}:", export_name, boost::describe::enum_to_string(device, ""));
        return fd;
    }
