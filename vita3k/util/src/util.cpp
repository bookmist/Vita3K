...
void float_to_half_AVX2(const float *src, std::uint16_t *dest, const int total) {
...
void float_to_half(const float *src, std::uint16_t *dest, const int total) {
    float_to_half_AVX2(src, dest, total);
}
#else
//check and use AVX2 if possible
...
int instrset_detect(void) {
    ...
    if ((abcd[2] & (1 << 26)) == 0)
        return iset; // no XSAVE //https://stackoverflow.com/questions/57796381/is-it-possible-that-speculative-execution-on-intel-cpu-causing-exc-bad-instructi
    if ((abcd[2] & (1 << 27)) == 0)
...
void float_to_half_init(const float *src, std::uint16_t *dest, const int total) {
    if (instrset_detect() >= 8) {
        float_to_half_var = float_to_half_AVX2;
        LOG_INFO("AVX2 supported. Use fast f32 to f16 conversion");
    } else {
        float_to_half_var = float_to_half_basic;
        LOG_INFO("AVX2 not supported. Use basic f32 to f16 conversion");
    }
