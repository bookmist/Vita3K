...
void float_to_half_F16C(const float *src, std::uint16_t *dest, const int total) {
...
#ifdef __AVX2__
//forced use F16C instruction set
//AVX2 checked intentionally cause checking AVX is not enough for some CPU architectures (Sandy bridge)
void float_to_half(const float *src, std::uint16_t *dest, const int total) {
    float_to_half_F16C(src, dest, total);
}
#else
//check and use F16C instruction set if possible
...
void float_to_half_init(const float *src, std::uint16_t *dest, const int total) {
    if (util::instrset::hasF16C()) {
        float_to_half_var = float_to_half_F16C;
        LOG_INFO("F16C instruction set supported. Use fast f32 to f16 conversion");
    } else {
        float_to_half_var = float_to_half_basic;
        LOG_INFO("F16C instruction set not supported. Use basic f32 to f16 conversion");
    }
