...
void update_live_area_current_open_apps_list(GuiState &gui, EmuEnvState &emuenv, const std::string &app_device, const std::string &app_path) {
    ...
    if (gui.live_area_current_open_apps_list.size() > 6) {
        const auto last_app = gui.live_area_current_open_apps_list.back().second == emuenv.io.app_path ? 
            gui.live_area_current_open_apps_list[gui.live_area_current_open_apps_list.size() - 2].second :
            gui.live_area_current_open_apps_list.back().second;
...
void pre_run_app(GuiState &gui, EmuEnvState &emuenv, const std::string &app_device, const std::string &app_path) {
    if (true || (app_path == "NPXS19999") || (app_path == "NPXS10062") /*|| (app_path.find("NPXS") == std::string::npos)*/) {
...
void draw_app_close(GuiState &gui, EmuEnvState &emuenv) {
    ...
    if (ImGui::Button(common["ok"].c_str(), BUTTON_SIZE)) {
        ...
        close_and_run_new_app(gui, emuenv, app_device, app_path);
    }
    ImGui::PopStyleVar();
    ImGui::EndChild();
    ImGui::PopStyleVar();
