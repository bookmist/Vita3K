...
static ExitCode load_app_impl(SceUID &main_module_id, EmuEnvState &emuenv, const std::wstring &path) {
    ...
    if (process_param) {
        auto PreloadDisabledPtr = Ptr<SceUInt32>(process_param->process_preload_disabled);
        if (PreloadDisabledPtr && PreloadDisabledPtr.valid(emuenv.mem)) {
            process_preload_disabled = *PreloadDisabledPtr.get(emuenv.mem);
        }
    }
